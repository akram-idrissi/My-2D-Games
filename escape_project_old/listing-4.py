...
top_left_x = 100 # the x position from where we'll drawing
top_left_y = 150 # the y position from where we'll drawing
...
GAME_MAP = [ ["Room 0 - where unused objects are kept", 0, 0, False, False] ] # appending the room 0 to the GAME_MAP list
...
GAME_MAP += [
        ...
        ["West corridor", 15, 5, True, True], # room 3
...
assert len(GAME_MAP)-1 == MAP_SIZE, f"Map size and GAME_MAP don't match, room number: {len(GAME_MAP) - 1}"
...
def generate_map(): 
    ...
    if room_data[4]: # if exit at the right of this room
        room_map[middle_row - 1][room_width - 1] = floor_type   # if you came across an error change the -1 
        room_map[middle_row ][room_width - 1] = floor_type      # to room_width - 1 (see below)
    ...
    if current_room <= MAP_SIZE - MAP_WIDTH:
        ...
        if room_below[3]:
            room_map[room_height - 1][middle_column - 1] = floor_type  # if you came across an error change the -1
            room_map[room_height - 1][middle_column] = floor_type  # to room_height - 1 (see above)
